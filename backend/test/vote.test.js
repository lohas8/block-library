...
// mock admin token (从 user.test.js 的流程获取)
...
describe('投票模块 API 测试', () => {
  ...
  describe('POST /api/votes - 创建投票（需管理员权限）', () => {
    it('管理员应能成功创建二选一投票', async () => {
      ...
      expect(response.body.title).toBe('健身器材更新采购方案');
      expect(Array.isArray(response.body.items)).toBe(true);
      expect(response.body.items.length).toBe(2);
      createdVoteId = response.body._id;
    });
  ...
  describe('GET /api/votes - 投票列表', () => {
    ...
    it('支持按状态筛选 active 投票', async () => {
      ...
      const activeVotes = response.body.list.filter(v => v.status === 'active');
      expect(activeVotes.length).toBe(response.body.list.length);
    });
  });
  ...
  describe('GET /api/votes/:id - 投票详情', () => {
    it('应返回投票详情（含选项）', async () => {
      ...
      expect(Array.isArray(response.body.items)).toBe(true);
      expect(response.body.items[0]).toHaveProperty('vote_count');
      expect(response.body.items[0]).toHaveProperty('label');
    });
  ...
  describe('POST /api/votes/:id/cast - 投票', () => {
    it('业主应能成功投票', async () => {
      if (!createdVoteId) return jest.skip();

      const response = await request(BASE_URL)
        .post(`/api/votes/${createdVoteId}/cast`)
        .set('Authorization', `Bearer ${userToken}`)
        .send({ selected_item_ids: [createdVoteId] }) // note: 实际应传 item._id
        .expect(200);
    ...
    it('重复投票应返回错误', async () => {
      ...
      expect(response.body.message).toContain('已投过');
    });
  });
  ...
  describe('POST /api/votes/:id/close - 结束投票（需管理员）', () => {
    it('管理员应能结束投票', async () => {
      ...
      expect(response.body.message).toContain('结束');
    });
  ...
  describe('GET /api/votes - 多选投票', () => {
    it('应支持多选类型投票', async () => {
      ...
      expect(response.body.vote_type).toBe('multi');
